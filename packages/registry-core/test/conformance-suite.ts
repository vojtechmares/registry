 ...
 * as one definition parameterized over the core context.
 *
 ...
 * gives a fast, precise signal on the logic itself - and, because the suite is
 * run once against the in-memory adapters and once against the real D1/R2/DO
 * adapters, on any divergence between the two.
 */

import { beforeEach, describe, expect, it } from "vitest";
import { MEDIA_TYPE_OCI_INDEX, MEDIA_TYPE_OCI_MANIFEST, sha256Hex } from "@registry/oci";
import { commitBlob } from "../src/commit.js";
import type { RegistryConfig, RegistryContext } from "../src/ports.js";
...
const digestOfBytes = (input: Uint8Array): string => `sha256:${sha256Hex(input)}`;
...
/**
 * Builds a fresh core context - one adapter set's implementation of the ports.
 * Called before every test and again wherever a test needs different config or
 * an authorizer, so the same suite runs unchanged against the in-memory adapters
 * and the real D1/R2/DO adapters.
 */
export type MakeRegistry = (
  overrides?: Partial<RegistryConfig>,
  options?: { partSize?: number; authorize?: RegistryContext["authorize"] },
) => RegistryContext | Promise<RegistryContext>;

/** What an adapter set can be asked to do; the real Durable Object fixes its part size. */
export interface Capabilities {
  /** Whether the upload store honours a custom multipart part size. */
  readonly customPartSize: boolean;
}

/**
 * The distribution-spec behaviours the official Go conformance suite asserts,
 * as one definition run against whichever adapter set `makeRegistry` builds. The
 * suite drives everything through `handleRegistryRequest`, asserts the port
 * contract, and never inspects a storage-key layout - so an adapter that
 * diverges fails here rather than only in the deployed Go suite.
 */
export function conformanceSuite(makeRegistry: MakeRegistry, capabilities: Capabilities): void {
  let registry: RegistryContext;
  ...
  async function call(method: string, path: string, init: RequestInit = {}): Promise<Response> {
    ...
    // Drain the body into memory and hand back a response over the buffer. A real
    // R2 range read hands out a stream that keeps a storage handle open until it is
    // consumed; a header-only assertion would otherwise leave it dangling and the
    // workerd test pool could not roll the isolated storage back. Harmless for the
    // in-memory adapters, which hold their bytes in memory already. A 204 or 304
    // carries no body, and the Response constructor forbids giving it one.
  ...
  describe("pull (end-2, end-3)", () => {
    ...
    it("honours Range requests on blobs", async () => {
      ...
      const clamped = await call("GET", `/v2/${NAME}/blobs/${digest}`, {
  ...
  // A custom part size lets one test exercise the multipart path without a 5 MiB
  // upload; the real Durable Object fixes its part size, so this runs on the
  // in-memory adapters alone. The behaviour it guards - a blob reassembled across
  // R2 part boundaries - is what the deployed Go suite exercises at full scale.
  describe.skipIf(!capabilities.customPartSize)("push: chunked across R2 part boundaries", () => {
    // A tiny part size exercises the multipart path without allocating 5 MiB.
    const partSize = 1024;

    beforeEach(async () => {
      registry = await makeRegistry({}, { partSize });
    });
  ...
  describe("cross-repository blob mount (end-11)", () => {
    ...
    it("opens a session instead when automatic cross-mount is disabled", async () => {
      registry = await makeRegistry({ automaticCrossMount: false });
    ...
    it("automatically mounts a blob the caller could already pull", async () => {
      registry = await makeRegistry(); // permissive authorizer
  ...
  describe("manifest push validation (end-7)", () => {
    ...
    it("rejects a manifest larger than the configured limit with 413", async () => {
      registry = await makeRegistry({ maxManifestSize: 128 });
    ...
    it("checks index children when configured to", async () => {
      registry = await makeRegistry({ validateManifestReferences: true });
      const document = {
        schemaVersion: 2,
        mediaType: MEDIA_TYPE_OCI_INDEX,
        manifests: [{ mediaType: MEDIA_TYPE_OCI_MANIFEST, digest: DUMMY_DIGEST, size: 10 }],
  ...
  describe("content management (end-9, end-10)", () => {
    it("deletes a tag, leaving the manifest reachable by digest", async () => {
      ...
      expect(
        ((await (await call("GET", `/v2/${NAME}/tags/list`)).json()) as { tags: string[] }).tags,
      ).toEqual([]);
    });
    ...
    it("answers 400 UNSUPPORTED when deletes are disabled", async () => {
      registry = await makeRegistry({ enableDeletes: false });
  ...
  describe("deduplication", () => {
    it("stores identical content once and links it from both repositories", async () => {
      const payload = deterministic(50, 12);
      const digest = await seedBlob(NAME, payload);
      await seedBlob(CROSSMOUNT, payload);

      // Both repositories serve the identical bytes back - the observable contract
      // of deduplication, asserted without reaching into the store's key layout.
      for (const repository of [NAME, CROSSMOUNT]) {
        const response = await call("GET", `/v2/${repository}/blobs/${digest}`);
        expect(response.status).toBe(200);
        expect(new Uint8Array(await response.arrayBuffer())).toEqual(payload);
      }
    });

    it("drops the object a losing writer produced, through the commit module", async () => {
      // The commit protocol both adapters share: register and link, then, when the
      // same bytes already won under another key, delete the object just written.
      // Keys are chosen here rather than derived, so nothing depends on a layout.
      const payload = deterministic(40, 13);
      const digest = digestOfBytes(payload);
      const hex = sha256Hex(payload);
      const winnerKey = "conformance/winner-object";
      const loserKey = "conformance/loser-object";

      // An incumbent already holds the bytes under its own key.
      await registry.content.put(winnerKey, payload, payload.length, hex);
      await registry.metadata.registerAndLinkBlob(NAME, {
        digest,
        size: payload.length,
        storageKey: winnerKey,
      });

      // A second write stores its own object, then loses the dedup race.
      await registry.content.put(loserKey, payload, payload.length, hex);
      const survivor = await commitBlob(registry.metadata, registry.content, CROSSMOUNT, {
        digest,
        size: payload.length,
        storageKey: loserKey,
      });

      // The incumbent survives; exactly the losing object is reclaimed. The
      // surviving body is cancelled rather than read, so a real R2 handle is
      // released before the test ends.
      expect(survivor.storageKey).toBe(winnerKey);
      const winner = await registry.content.get(winnerKey);
      expect(winner).not.toBeNull();
      await winner?.body.cancel();
      expect(await registry.content.get(loserKey)).toBeNull();

      // And both repositories serve the blob from the surviving object.
      expect((await call("GET", `/v2/${NAME}/blobs/${digest}`)).status).toBe(200);
      expect((await call("GET", `/v2/${CROSSMOUNT}/blobs/${digest}`)).status).toBe(200);
    });
  });
